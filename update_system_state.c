...
void update_system_state(SystemInputState *input_state, SystemOutputState *output_state) {
    ...
    // we are only allowed to charge one battery at a time
    // keep track of whether we are already charging a battery
    ...
    } else if (top_port_state == PORT_STATE_EXTERNAL_BATTERY_REQUESTING_POWER &&
               battery_percentage >= HIGH_BATTERY_PERCENTAGE && is_charging_external_battery == false) {
        output_state->top_output_enable = HIGH;
    ...
    if (bottom_port_state == PORT_STATE_EXTERNAL_BATTERY_REQUESTING_POWER &&
        battery_percentage >= HIGH_BATTERY_PERCENTAGE && is_charging_external_battery == false) {
